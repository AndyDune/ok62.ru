...
class Dune_Image_Info
{
    ...
    public function __construct($file)
    {
		if (is_file($file))
		{
		    ...
		    if (phpversion() < '5.2')
		    {
		    	$this->fileArray['filename'] = substr($this->fileArray['basename'], 0, strpos($this->fileArray['basename'], '.')) ;
		    }
...
*/
    		if ($this->imageProperties = getimagesize($file))
    		{
    		    ...
    		    $this->extension = image_type_to_extension($this->imageProperties[2]);
                if ($this->extension == '.jpeg')
                    $this->extension = '.jpg';
                $this->extensionNoDot = str_replace('.', '', $this->extension);
