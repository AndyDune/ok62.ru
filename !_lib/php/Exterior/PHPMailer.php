...
/*
    ...
    Интерфейс класса переработан.
    НЕСОВМЕСТИМ С ОРИГИНАЛЫМ КЛАССОМ.
...
Доработан для удобства.
Доьбавлен механизм цепочек для set-методов.
...
class Exterior_PHPMailer {
  ...
  /////////////////////////////////////////////////
    ...
    public function __construct($char_set = 'windows-1251')
    {
     ...
     * @param  string    $email
     * @param  string    $name
     * @return Dune_Mail Provides fluent interface
     * @throws Dune_Exception_Base if called subsequent times
     */
    public function setFrom($email, $name = '')
    {
        ...
        {
            throw new Dune_Exception_Base('From Header set twice');
        }
        return $this;
    }
    ...
    {
        if ($this->Body and $this->ContentType == 'text/plain')
        {
    ...
    {
        if ($this->Body and $this->ContentType == 'text/html')
        {
            $this->AltBody = $this->Body;
            $this->ContentType = 'multipart/alternative';
        }
    ...
    /**
     * Устанавливает параметры передачи по smtp
     *
     * @param string $host 
     * @param integer $port
     * @param string $secure Бывает "", "ssl" или "tls"
     */
    public function setSmtp($host, $port = null, $secure = '')
    {
        $this->isSMTP();
        $this->Host = $host;
        if ($port)
    ...
    /**
     * Устанавливает SMTP-аутентификацию.
     *
   ...
   * characters.
   */
    public function setWordWrap($value = 50)
    {
        $mail->WordWrap = $value;
        return $this;
    }  
...
////////////        Не трогались имена методов  
  ...
  public function SmtpSend($header, $body) {
    include_once('Exterior/PHPMailer/SMTP.php');
    $error = '';
    $bad_rcpt = array();
  ...
  public function SmtpConnect() {
    if($this->smtp == NULL) {
      $this->smtp = new Exterior_PHPMailer_SMTP();
    }
  ...
  public function UTF8CharBoundary($encodedText, $maxLength) {
   ...
   /**
    * Далает перенос строки в текстовом сообщении, если было установлено.
    * @access private
    * @return void
    */
    protected function _setWordWrap()
    {
        if($this->WordWrap < 1)
        {
            return;
