...
try {
    ...
    {
        $post->trim();
        $require_array = array('adress_code', 'adress_id');
        $stop = false;
        $x = 0;
        ...
        $session->killZone();
        throw new Dune_Exception_Control_Goto($folder_url->getExeptLastFolder(2) . 'edit/'. $object->add() . '/');
...
//    $view->form_action = $folder_url->get() . $URL[6] . '/';
    ...
    echo $view->render('page/catalogue/object/add/type_garage');
}
