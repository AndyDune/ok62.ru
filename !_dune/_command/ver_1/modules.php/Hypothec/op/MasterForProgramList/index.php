...
$pagetitle = "Результаты работы мастера подбора ипотечной прораммы";
...
if ((int)$o_session->age > 25)
{
        $sql_filter_age = ' AND `pp`.`pprogram_id` IN 
                        (
                        SELECT DISTINCT pp.pprogram_id 
                        FROM `hypothec_pprograms` pp, `hypothec_programs` p
                        WHERE pp.`creditperiod_min_pp` < p.agemax - ' . (int)$o_session->age . ' 
                        )';
        
        $sql_filter_age = ' AND pp.`creditperiod_min_pp` < p.agemax - ' . (int)$o_session->age . ' 
                        ';
...
$m_list->filter_add = $sql_filter_age;
...
{
    $navigation = new Dune_Navigate_Page('/modules.php?name=Hypothec&op=MasterForProgramList&page=', $nav_count_total, $get->getDigit('page'), $nav_count_per_page);
...
echo $view->render('hypothec:MasterForProgramList');
