...
///////////////////////////////
////     
///     Добавление улицы небольшому городу.
///
...
try {
    ...
    if ($post->_do_ == 'save')
    {
        ...
        $data_settlement = $DB->query($q, array($post->settlement_id), Dune_MysqliSystem::RESULT_ROWASSOC);
        if (!$data_settlement)
            throw new Dune_Exception_Control_Goto($folder_url->getExeptLastFolder(2));
        ...
        $set_q->assign('district_id', 0, 'i');
        $set_q->assign('settlement_id', $data_settlement['id'], 'i');
    ...
    $crumbs->addCrumb('Улицы: ' . $data_settlement['name'], $link_list_settlement);
    $crumbs->addCrumb('Добавить улицу');
    ...
    echo $view->render('page/catalogue/adress/add/street');
}
