...
class Module_Display_Catalogue_Adress_ListMainWithObjectsPlus extends Dune_Include_Abstract_Code
{
...
$query_from = '';
$query_where_edinstvo = '';
$query_where = '';
$session = Dune_Session::getInstance($this->session_zone);
if (Special_Vtor_Object_List::$edinstvo)
{
    $query_where .= ' AND objects.edinstvo = 1';
    $query_where_edinstvo = ' AND objects.edinstvo = 1';
}
...
$user = $vars->subdomainFocusUserId;

if ($vars->subdomainFocusUserId)
{
    $q_user_objects = ' AND objects.saler_id = ' . $vars->subdomainFocusUserId;
    $q_user_object = ' AND objects.saler_id = ' . $vars->subdomainFocusUserId;
}
else 
{
    $q_user_objects = '';
...
$list_area = array(); // Список районов областей
...
$list_settlement_no_ryazan = array();
        ...
        $q = 'SELECT sett.*, count(objects.id) as count,
              ...
              WHERE objects.`region_id` > 1
                    ...
                    AND
                    objects.activity = 1
                    ' . $q_user_objects . $query_where_edinstvo . '
              GROUP BY sett.id
              ';
...
Dune_Zend_Cache::$allowLoad = $store_cache_load;
    ...
    $text = Dune_Zend_Cache::loadIfAllow('adress_have_region_center_url_plus_' . $have_region_center_url);
    if (!$text)
    {
        $q = 'SELECT dist.*, count(objects.id) as count
              FROM `unity_catalogue_adress_district_plus` as dist,
                   `unity_catalogue_object` as objects
              WHERE dist.`settlement_id` = ?i
                    AND
                    dist.id = objects.district_id_plus
                    AND
                    objects.activity = 1
                    ' . $q_user_object . $query_where_edinstvo . '                    
              GROUP BY dist.id
              ORDER BY dist.order, dist.name              
              ';
        $from_db = $BD->query($q, array($have_region_center_url), Dune_MysqliSystem::RESULT_IASSOC);
        if ($from_db)
            Dune_Zend_Cache::saveIfAllow(serialize($from_db), 'adress_have_region_center_url_plus_' . $have_region_center_url,  array(Special_Vtor_Settings::CACHE_TAG_CATALOGUE));
        }
        else 
        {
            $from_db = unserialize($text);
...
Dune_Zend_Cache::$allowLoad = $store_cache_load_local;
    ...
    foreach ($from_db as $value)
    {
        ...
        $cat_url->setDistrict($value['id'], true);
        $arr['link'] = $cat_url->get();
        $arr['name'] = $value['name'];
    ...
    if (isset($list_district[16]) and isset($list_district[10])) // Малыйцентр - часть большого
    {
        $list_district[16]['count'] += $list_district[10]['count'];
    }
...
//////////////////////////////////////////////////////////////////////////////////////  begin
////                         Формирование списка улиц в районе города   !!!!  Без учета таблицы раонов
...
Dune_Zend_Cache::$allowLoad = $store_cache_load;
            ...
            if ($have_district_in_url == 16) // Малый центр - часть большого
                $str = ' IN (10, 16)';
            else
                $str = ' = ' . $have_district_in_url;

        
        $text = Dune_Zend_Cache::loadIfAllow('adress_have_district_in_url_plus_' . $have_district_in_url);
        if (!$text)
        {
            $q = 'SELECT street.*, count(objects.id) as count,
                    objects.district_id_plus as district_id_plus
            ...
            $from_db = $BD->query($q, null, Dune_MysqliSystem::RESULT_IASSOC);
            if ($from_db)
                Dune_Zend_Cache::saveIfAllow(serialize($from_db), 'adress_have_district_in_url_plus_' . $have_district_in_url,  array(Special_Vtor_Settings::CACHE_TAG_CATALOGUE));
...
//        $cat_url->setDistrict($have_district_in_url, true);
